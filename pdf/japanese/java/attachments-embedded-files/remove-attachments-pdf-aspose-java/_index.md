...
Document pdfDocument = new Document(dataDir);
```
- **Why**: このステップでドキュメントの内容にアクセスし、操作できるようになります。
...
```
- **Why**: `delete()` メソッドは不要なデータをすべてクリアし、PDF をクリーンかつ最適化された状態にします。
...
```
- **Why**: 保存により変更が永続化され、**save pdf without attachments**（添付ファイルなしの PDF）バージョンが配布可能になります。
