...
4. Образец файла PDF: Вам понадобится образец документа PDF с тегами для работы. Для этого урока мы будем использовать "StructureElementsTree.pdf", который вы должны поместить в каталог документов вашего проекта.
...
Давайте разобьем эту задачу на выполнимые шаги.
...
Теперь, когда мы получили доступ к корневым элементам, давайте рассмотрим подробнее первый корневой элемент, чтобы получить доступ к его дочерним элементам.
...
Это как будто придать новый облик каждому выбранному элементу конструкции!
...
Тегированный PDF-файл — это документ, содержащий метаданные, обеспечивающие лучшую доступность и навигацию.
