...
```
В этом примере мы добавляем изображение на первую страницу PDF-файла (Pages[1] относится к первой странице, поскольку индексация начинается с единицы).
...
```
Этот шаг гарантирует, что любые изменения, внесенные в состояние графики (например, преобразования или масштабирование), будут отменены, оставляя остальную часть документа неизменной.
...
Вставка изображения в файл PDF с помощью Aspose.PDF для .NET проста, если разбить процесс на шаги. Используя различные операторы, такие как `GSave`, `ConcatenateMatrix`, и `Do`, вы можете легко контролировать размещение и отображение изображений в ваших PDF-документах. Этот метод необходим для настройки и брендирования PDF-файлов с помощью логотипов, водяных знаков или любых других изображений.
