...
```
В этом фрагменте мы определяем путь к нашему каталогу документов и создаем экземпляр `Document` класс с использованием образца PDF-файла с именем `blank.pdf`. Обязательно замените `"YOUR DOCUMENTS DIRECTORY"` с фактическим путем расположения вашего PDF-файла.
