...
Чтобы импортировать их, добавьте приведенные выше строки в начало вашего файла C#. Это предоставит вам доступ к классам и методам, которые мы будем использовать для управления вашими PDF-документами.
...
Document doc = new Document(dataDir + "ЗаменятьTextPage.pdf");
```

Replace `"YOUR DOCUMENT DIRECTORY/"` с фактическим путем, где хранится ваш PDF-файл. Этот шаг имеет решающее значение, поскольку он позволяет Aspose получить доступ к вашему PDF-документу. 
...
После определения фрагментов текста, пришло время пройтись по ним и заменить все неиспользуемые шрифты. Добавьте этот код:
