...
Первое, что нам нужно сделать, это определить путь к каталогу, где хранятся наши PDF-файлы. Это место, где будет находиться PDF-файл, подлежащий проверке, и где будут сохраняться результаты проверки.
...
После того, как вы задали путь к файлу, следующим шагом будет открытие документа PDF, который вы хотите проверить. Aspose.PDF упрощает загрузку документа с помощью `Document` сорт.
