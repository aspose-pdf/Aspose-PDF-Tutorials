...
Здесь мы получаем доступ к полю формы с именем `"textbox1"`. Убедитесь, что вы знаете имя поля формы, которым хотите управлять, или при необходимости вы можете использовать другие методы для составления списка или поиска по полям формы.
