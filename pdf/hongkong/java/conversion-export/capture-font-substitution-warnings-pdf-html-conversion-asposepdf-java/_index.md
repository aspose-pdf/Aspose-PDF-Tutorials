...
### 功能：PDF 轉 HTML 轉換中的字體替換警告

此功能可讓您監控並擷取 PDF 轉 HTML 轉換過程中發生的任何字型替換。

#### 步驟 1：載入 PDF 文檔

（如上所示）載入文件後，您可以存取其內容和字體資訊。

#### 步驟 2：設定字型替換處理程序

註冊一個處理程序，將每次替換記錄到一個映射表中，以便稍後檢查。
...
#### 步驟 3：設定 HTML 儲存選項
建立一個 `HtmlSaveOptions` 實例來控制 PDF 儲存為 HTML 的方式。
