...
Các `Pages.Add()` phương pháp này tạo ra một trang mới trong tài liệu PDF của chúng ta. Hãy nghĩ về trang này như một trang vải trắng nơi hình ảnh sẽ xuất hiện.
...
Các `Save()` phương pháp này sẽ xuất ra tệp PDF với tên đã chỉ định. Ở đây, tệp PDF được lưu dưới dạng `ConvertMemoryStreamImageToPdf_out.pdf` trong cùng thư mục với tệp hình ảnh.
