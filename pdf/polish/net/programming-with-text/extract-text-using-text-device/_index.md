...
4. Plik wejściowy PDF: Plik PDF, którego będziesz używać do ekstrakcji tekstu. Umieść go w katalogu na swoim komputerze (będziemy go nazywać `YOUR DOCUMENT DIRECTORY`).
...
Tutaj używamy `Document pdfDocument = new Document(dataDir + "input.pdf");` aby załadować plik PDF. `dataDir` zmienna przechowuje ścieżkę katalogu pliku PDF. Da nam to dostęp do całego dokumentu, umożliwiając nam przeglądanie stron i wyodrębnianie treści.
...
Teraz skonfigurujemy proces ekstrakcji tekstu dla każdej strony. Tutaj utworzymy `TextDevice` obiekt i użyj go do przetworzenia stron PDF. `TextDevice` wyodrębnia surowy lub sformatowany tekst na podstawie ustawionych przez nas opcji wyodrębniania.
...
Ekstrakcja tekstu z pliku PDF przy użyciu Aspose.PDF dla .NET to prosty i wydajny proces. Postępując zgodnie z krokami opisanymi w tym przewodniku, możesz łatwo otwierać dokumenty PDF, przechodzić przez strony i ekstrahować tekst do pliku tekstowego. Jest to szczególnie przydatne do przetwarzania dużych ilości danych PDF, wykonywania analizy tekstu lub konwertowania dokumentów w celu dalszej manipulacji.
...
Tak, możesz zmodyfikować kod, aby pętla działała na określonym zakresie stron, a nie na wszystkich stronach.
