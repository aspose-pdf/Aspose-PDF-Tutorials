...
```
tym fragmencie kodu definiujemy ścieżkę do naszego katalogu dokumentów i tworzymy wystąpienie `Document` klasa korzystająca z przykładowego pliku PDF o nazwie `blank.pdf`. Upewnij się, że wymienisz `"YOUR DOCUMENTS DIRECTORY"` z rzeczywistą ścieżką, gdzie znajduje się Twój plik PDF.
...
using (Facades.PdfFileSignature pdfSign = new Facades.PdfFileSignature())
{
    pdfSign.BindPdf(doc);
```
Tutaj tworzymy instancję `PdfFileSignature` i powiąż go z naszym dokumentem PDF. To przygotowuje dokument do podpisania.
...
```
W tym kroku wywołujemy `Sign` metoda na naszej `pdfSign` obiekt. Oto co oznacza każdy parametr:
...
Certyfikat cyfrowy można uzyskać od zaufanego urzędu certyfikacji (CA) lub wykorzystać certyfikat podpisany samodzielnie w celach testowych.
