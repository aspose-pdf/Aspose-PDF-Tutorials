...
Pamiętaj o wymianie `"YOUR DOCUMENT DIRECTORY"` z rzeczywistą ścieżką do folderu dokumentów. Ta zmienna będzie przydatna w całym procesie konwersji.
...
Podpowiedzi dotyczące czcionek to sprytna funkcja, która pomaga poprawić przejrzystość czcionek w konwertowanych obrazach. Aby to umożliwić, utworzymy `RenderingOptions` obiekt i zbiór `UseFontHinting` Do `true`:
...
Następnie musimy skonfigurować urządzenie PNG. Obejmuje to określenie rozdzielczości i zastosowanie opcji renderowania ustawionych wcześniej. Zróbmy to:
