...
在数字时代，将 HTML 文档转换为 PDF 是一项常见的任务。然而，有时您可能出于各种原因（例如增强可读性或防止不必要的导航）想要从转换后的 PDF 中删除超链接。在本教程中，我们将探索如何使用 Aspose.PDF for .NET 实现此目的。 
...
接下来，您将使用 `Document` 来自 Aspose.PDF 的类。该类允许您轻松处理 PDF 文档。

```csharp
Document doc = new Document(dataDir + "SampleHtmlFile.html", new HtmlLoadOptions());
```

这里，我们加载名为 `SampleHtmlFile.html`确保此文件存在于您指定的目录中。
...
此行将文档保存在内存中，使我们能够使用它而无需将其写入磁盘。

## 步骤 4：迭代注释

现在，我们将遍历文档中的注释。注释包括链接、注释和高亮等元素。我们特别关注的是链接注释。
...
在这个循环中，我们检查注释类型是否为链接。如果是，则继续下一步。

## 步骤5：删除超链接操作

对于每个链接注释，我们需要检查它是否具有超链接操作。如果有，我们将通过将其 URI 设置为空字符串来删除该超链接。
...
获得文本片段后，我们就可以修改它们的外观。在本例中，我们将删除下划线并将文本颜色更改为黑色。
...
此行从 PDF 中删除超链接，确保它不再存在于最终输出中。

## 步骤9：保存修改后的文档

最后，我们需要将修改后的文档保存为一个新的PDF文件。这是我们流程的最后一步。

```csharp
doc.Save(dataDir + "RemoveHyperlinksFromText_out.pdf");
```

此行保存文档并删除超链接，创建一个名为 `RemoveHyperlinksFromText_out。pdf`.

## 结论

就这样！使用 Aspose.PDF for .NET 将 HTML 文档转换为 PDF 后，您已成功删除其中的超链接。此过程不仅增强了 PDF 的可读性，还让您能够更好地控制所呈现的内容。 
...
### Aspose.PDF 可以免费使用吗？
Aspose.PDF 提供免费试用，但要获得完整功能，您需要购买许可证。请查看 [购买页面](https://purchase。aspose.com/buy).

### 如果我在使用 Aspose.PDF 时遇到问题怎么办？
您可以在 [支持论坛](https://forum。aspose.com/c/pdf/10).

### 我可以使用 Aspose 将其他文件格式转换为 PDF 吗？
